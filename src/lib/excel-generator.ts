...
export function generateExcelFileForRoleTemplates(
  forms: { rows: RoleTemplateExcelField[]; title: string }[]
) {
  // Create a new workbook
  const workbook = XLSX.utils.book_new();
  ...
  forms.forEach(({ rows, title }, index) => {
    // Prepare the data for the worksheet
    const worksheetData = [
      ["Role", "Web Modules", "Mobile Modules"],
      ...rows.map((row) => [row.roleTitle, row.webModules, row.mobileModules]),
    ];
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(
        workbook,
        worksheet,
        title ? String(title).slice(0, 31) : "Role_" + (index + 1)
      );
    } catch (error) {
      console.error("Error adding sheet: ", error);
      XLSX.utils.book_append_sheet(
        workbook,
        worksheet,
        title ? String(title).slice(-31) : "Role_" + (Math.random() * 1000 + 1)
      );
    }
  });
